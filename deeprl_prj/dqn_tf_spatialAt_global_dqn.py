...
class Qnetwork():
    def __init__(self, args, h_size, num_frames, num_actions, myScope):
        ...
        context = self._attention_layer(self.features, myScope=myScope)

        self.rnn = tf.contrib.layers.fully_connected(context, 512, activation_fn=tf.nn.relu)
    ...
    def _attention_layer(self, features, myScope, reuse=False):
        with tf.variable_scope(myScope+'_attention_layer', reuse=reuse):
            #w = tf.get_variable('w', [self.H, self.D], initializer=self.weight_initializer)
            b_att = tf.get_variable('b', [1], initializer=self.const_initializer)
            w_att = tf.get_variable('w_att', [self.D, 1], initializer=self.weight_initializer)

            b_att_c = tf.get_variable('b_c', [1], initializer=self.const_initializer)
            w_att_c = tf.get_variable('w_att_c', [self.L, 1], initializer=self.weight_initializer)


            #h_att = tf.nn.relu(features_proj + tf.expand_dims(tf.matmul(h, w), 1) + b)    # (N, L, D)
            a = tf.reshape(tf.nn.tanh(tf.matmul(tf.reshape(features, [-1, self.D]), w_att) + b_att), [-1, self.L])   # (N, L)
            alpha = tf.nn.softmax(a)
            context = features * tf.expand_dims(alpha, 2)   #(N, D)

            #context = tf.transpose(context, [0, 2, 1])

            #b = tf.reshape(tf.nn.tanh(tf.matmul(tf.reshape(context, [-1, self.L]), w_att_c) + b_att_c),[-1, self.D])  # (N, L)
            #beta = tf.nn.softmax(b)
            #context = context * tf.expand_dims(beta, 2)  # (N, D)

            context = tf.reshape(context, [-1, self.L * self.D])
...
class DQNAgent:
    ...
    def evaluate(self, env, num_episodes, eval_count, max_episode_length=None, monitor=True):
        ...
        with open('reward_mean_global_dqn.txt','a') as file:
